...
import lu.mobilab.playmobel.backend.util.javagmm.GMMConfig;
import lu.mobilab.playmobel.backend.util.javagmm.GMMJava;
import org.mwg.*;
                    ...
                    .forEach(newTask()
                            .thenDo(new ActionFunction() {
                                ...
                                public void eval(TaskContext context) {
                                    ...
                                    TreeMap<Long, double[]> userMap=new TreeMap<Long, double[]>();

                                    for (int i = 0; i < 24 * 7; i++) {
                                        GMMJava profiler=new GMMJava(config);
...
//                                        profiler.free();
                                    ...
                                    context.defineVariable("userMap", userMap);
                                                ...
                                                public void eval(TaskContext ctx) {
                                                    ...
                                                    if (numOfLine >= 6) {
                                                       ...
                                                       // Node user = (Node) ctx.variable("user").get(0);
                                                        TreeMap<Long, double[]> userMap=(TreeMap<Long, double[]>)ctx.variable("userMap").get(0);
...
//                                                        final double lng = Double.parseDouble(substr[1]);
                                                        ...
                                                        GMMJava profiler = (GMMJava) ctx.variable("profiler" + relid).get(0);

                                                        int globalCounter = (int) ctx.variable("dataload").get(0);
                                                        globalCounter++;
                                                        ctx.setGlobalVariable("dataload", globalCounter);

                                                        userMap.put(timestamp,latlng);
                                                        profiler.learnVector(latlng);
                                                        ctx.continueTask();
...
//                                                                profiler.learnVector(new double[]{lat, lng});
//                                                                ctx.continueTask();
